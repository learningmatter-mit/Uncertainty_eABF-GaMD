...
from eabfgamd.ala import Paths
from eabfgamd.enhsamp import check_completion, make_config_for_each_simulation
...
def write_enhsamp_job(
    config: dict, config_path: str, force_restart: bool = False
) -> str:
...
def write_calc_job(config: dict, config_path: str) -> str:
    ...
    with open(script_path, "w") as f:
        ...
        fwrite(f"source activate openmm && {chr(92)}")
        fwrite(f"python $PROJDIR/single_point_calc.py $config_path && {chr(92)}")
        fwrite(f"conda deactivate && {chr(92)}")
...
def write_aggregate_job(config: dict, config_path: str) -> str:
    main_outdir = get_main_model_outdir(config["model"]["outdir"])
...
def handle_train(
    config: dict, restore: bool = False, force_retrain: bool = False
) -> List[str]:
    ...
    for config_file in config_files:
        ...
        script_path = write_mace_train_job(config_file)
...
def handle_enhsamp(config: dict, force_restart: bool = False) -> List[str]:
    # create a config for each simulation
    orig_config = deepcopy(config)
    config_files = make_config_for_each_simulation(orig_config)
    script_paths = []
    for config_file in config_files:
        # if force restart is enabled, recreate the directory without checking
        ...
        script_path = write_enhsamp_job(config_file, config_path, force_restart)
        script_paths.append(script_path)
...
def submit_individual_job(
    ...
    for job_type in job_types:
        assert job_type in ["train", "agg", "enhsamp", "calc"]
        ...
        if job_type == "train":
            ...
            if dry_run is False:
                for script_path in script_paths:
                    job_id = submit_job(script_path=script_path)
        ...
        # run enhanced sampling trajectories
            ...
            script_paths = handle_enhsamp(
                config,
                force_restart=enhsamp_force_restart,
            )
            if dry_run is False:
                job_ids = [submit_job(script_path) for script_path in script_paths]
...
def argument_parser():
    ...
    pipeline_parser.add_argument(
        "--enhsamp_force_restart", action="store_true", default=False
    )
    ...
    if args.command == "pipeline":
        ...
        for model_id in args.model_ids:
            ...
            pipeline(
                ...
                enhsamp_force_restart=args.enhsamp_force_restart,
